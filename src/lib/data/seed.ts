...
import type { Brand, BrandRecord, IngredientYield, OutletRecord, PackagingItem, RecipePackaging, RecipeVersion, RoleRecord, RawMaterial, Recipe, RecipeIngredient, SystemRole, User } from "./types";
import moinData from "./moin.generated.json";
...
const users: User[] = [
  { id: "u-owner", name: "M S Patel (Owner)", email: "mspatel05831@gmail.com", role: "super_admin", status: "active", approved: true, email_verified: true, dashboard_access: true, password: "password123", created_at: SEED_TS, updated_at: SEED_TS },
...
/**
 * First-run seed for the LIVE app: keep login + organisation structure (roles,
 * brands, outlets, system settings and a single Super-Admin account) but start
 * with an EMPTY catalog — no materials, recipes, prices, yields, packaging or
 * wastage. The full catalog is preserved in /data (see scripts/export-data.ts)
 * and remains available via buildSeed() for tests / restoring demo data.
 */
export function buildEmptySeed(): MockDb {
  const db = buildSeed();
  return {
    ...db,
    users: db.users.filter((u) => u.role === "super_admin"), // keep the owner only
    raw_materials: [],
    recipes: [],
    recipe_ingredients: [],
    recipe_cost_history: [],
    ...
    recipe_access_links: [],
    audit_logs: [],
    // kept from the full seed: brands, outlets, roles, system_settings
  };
}

/**
 * Initial catalog imported from "Reicpe for moin.xlsx" (generated by
 * scripts/build-moin-seed.mjs → moin.generated.json). Starts from the empty base
 * (login + brands/outlets/roles/settings) and loads the imported raw materials,
 * in-house prep sub-recipes, menu dishes and packaging. This is what the live app
 * seeds on first load.
 */
export function buildMoinSeed(): MockDb {
  const base = buildEmptySeed();
  return {
    ...base,
    raw_materials: structuredClone(moinData.raw_materials) as unknown as RawMaterial[],
    recipes: structuredClone(moinData.recipes) as unknown as Recipe[],
    recipe_ingredients: structuredClone(moinData.recipe_ingredients) as unknown as RecipeIngredient[],
    recipe_versions: structuredClone(moinData.recipe_versions) as unknown as RecipeVersion[],
    packaging_items: structuredClone(moinData.packaging_items) as unknown as PackagingItem[],
    recipe_packaging: structuredClone(moinData.recipe_packaging) as unknown as RecipePackaging[],
  };
}
