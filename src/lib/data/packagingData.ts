// AUTO-EXTRACTED from "BOOKENDS COSTING 16-05-2026 (1).xlsx" (gitignored source).
// Do not edit by hand — regenerate from the sheet. Packaging master items + each
// dish's authoritative packaging cost (₹) and finished weight (g).

export interface PackagingSeed { name: string; type: string; unit: string; price: number; }
export const PACKAGING_MASTER_SEED: PackagingSeed[] = [
  {
    "name": "1000 ML Round Container Transparent (RP)",
    "type": "primary",
    "unit": "Piece",
    "price": 9.85
  },
  {
    "name": "1000 ML Round Container Transparent (RP) x 2",
    "type": "primary",
    "unit": "Piece",
    "price": 19.7
  },
  {
    "name": "1000 ML Round Container Transparent (RP) X2",
    "type": "primary",
    "unit": "Piece",
    "price": 19.7
  },
  {
    "name": "300 ML Round Container Transparent (RP)",
    "type": "primary",
    "unit": "Piece",
    "price": 5.1
  },
  {
    "name": "500 ML Round Container Transparent (RP)",
    "type": "primary",
    "unit": "Piece",
    "price": 7.35
  },
  {
    "name": "500 Ml White Paper Noodle Box",
    "type": "primary",
    "unit": "Piece",
    "price": 6.78
  },
  {
    "name": "750 ML Round Container Transparent (RP)",
    "type": "primary",
    "unit": "Piece",
    "price": 9.19
  },
  {
    "name": "750 Ml White Paper Noodle Box",
    "type": "primary",
    "unit": "Piece",
    "price": 6.65
  },
  {
    "name": "AIKO Takeaway bag",
    "type": "secondary",
    "unit": "Piece",
    "price": 17.7
  },
  {
    "name": "Aiko Takeaway Glass",
    "type": "secondary",
    "unit": "Piece",
    "price": 14
  },
  {
    "name": "Boba Tea White straw",
    "type": "tertiary",
    "unit": "Piece",
    "price": 0.55
  },
  {
    "name": "Chopstick",
    "type": "tertiary",
    "unit": "Piece",
    "price": 1.95
  },
  {
    "name": "cup holder",
    "type": "secondary",
    "unit": "Piece",
    "price": 2.54
  },
  {
    "name": "Dp 2oz (50ml) Round Cont",
    "type": "primary",
    "unit": "Piece",
    "price": 1.77
  },
  {
    "name": "Dp 2oz (50ml) Round Cont X 2",
    "type": "primary",
    "unit": "Piece",
    "price": 3.54
  },
  {
    "name": "Fork spoon",
    "type": "tertiary",
    "unit": "Piece",
    "price": 0.63
  },
  {
    "name": "Napkins x 2",
    "type": "tertiary",
    "unit": "Piece",
    "price": 0.66
  },
  {
    "name": "Soup spoon",
    "type": "tertiary",
    "unit": "Piece",
    "price": 0.65
  },
  {
    "name": "Soy Sticker",
    "type": "tertiary",
    "unit": "Piece",
    "price": 5
  },
  {
    "name": "Soy Sticker x 2",
    "type": "tertiary",
    "unit": "Piece",
    "price": 10
  },
  {
    "name": "Soy Sticker x2",
    "type": "tertiary",
    "unit": "Piece",
    "price": 10
  },
  {
    "name": "spoon",
    "type": "tertiary",
    "unit": "Piece",
    "price": 0.63
  },
  {
    "name": "transparent stand-up zipper pouch",
    "type": "secondary",
    "unit": "Piece",
    "price": 4
  },
  {
    "name": "White spoon",
    "type": "tertiary",
    "unit": "Piece",
    "price": 0.63
  },
  {
    "name": "White straw",
    "type": "tertiary",
    "unit": "Piece",
    "price": 0.48
  }
];

/** normalized dish name -> { pkg: packaging cost ₹, weightG: finished weight g|null } */
export const RECIPE_PACKAGING_BY_NAME: Record<string, { pkg: number; weightG: number | null }> = {
  "margherita pizza": {
    "pkg": 30,
    "weightG": null
  },
  "sid s pizz": {
    "pkg": 30,
    "weightG": null
  },
  "peperone pizza": {
    "pkg": 30,
    "weightG": null
  },
  "prime hulk pizza": {
    "pkg": 30,
    "weightG": null
  },
  "baby hulk pizza": {
    "pkg": 30,
    "weightG": null
  },
  "mid hulk pizza": {
    "pkg": 30,
    "weightG": null
  },
  "third wave pizza": {
    "pkg": 30,
    "weightG": null
  },
  "garlic pie pizza": {
    "pkg": 30,
    "weightG": null
  },
  "truffle pizza": {
    "pkg": 30,
    "weightG": null
  },
  "rubirosa pizza": {
    "pkg": 30,
    "weightG": null
  },
  "ortolana pizza": {
    "pkg": 30,
    "weightG": null
  },
  "chilli crunch": {
    "pkg": 30,
    "weightG": null
  },
  "affair pizza": {
    "pkg": 30,
    "weightG": null
  },
  "apollo pizza": {
    "pkg": 30,
    "weightG": null
  },
  "triple sauce": {
    "pkg": 30,
    "weightG": null
  },
  "diavolo": {
    "pkg": 30,
    "weightG": null
  },
  "picante": {
    "pkg": 30,
    "weightG": null
  },
  "picanate": {
    "pkg": 24.46,
    "weightG": null
  },
  "burrata hot honey": {
    "pkg": 30,
    "weightG": null
  },
  "tofu xo pizza": {
    "pkg": 30,
    "weightG": null
  },
  "hot corn": {
    "pkg": 30,
    "weightG": null
  },
  "hell boy pizza": {
    "pkg": 30,
    "weightG": null
  },
  "chilli butter corn": {
    "pkg": 30,
    "weightG": null
  },
  "doughballs": {
    "pkg": 35.11,
    "weightG": 180
  },
  "garlic bread": {
    "pkg": 30.76,
    "weightG": 180
  },
  "caesar salad": {
    "pkg": 33.34,
    "weightG": 150
  },
  "butter garlic mushrooms": {
    "pkg": 28.99,
    "weightG": 250
  },
  "tiramisu": {
    "pkg": 26.19,
    "weightG": 215
  },
  "miso tomato soup": {
    "pkg": 33.34,
    "weightG": 200
  },
  "sticky toffee pudding": {
    "pkg": 31.77,
    "weightG": 200
  },
  "tomato butter risotto": {
    "pkg": 28.99,
    "weightG": 311
  },
  "pistachio mousse cake": {
    "pkg": 32.68,
    "weightG": 120
  },
  "saucy brussels": {
    "pkg": 33.34,
    "weightG": 233
  },
  "summer burrata salad": {
    "pkg": 33.34,
    "weightG": 349
  },
  "arancini": {
    "pkg": 28.99,
    "weightG": 170
  },
  "chocolate crunch cake": {
    "pkg": 32.68,
    "weightG": 162
  },
  "burrata salad": {
    "pkg": 33.34,
    "weightG": 300
  },
  "toamto burrata": {
    "pkg": 33.34,
    "weightG": 170
  },
  "tahina salad": {
    "pkg": 33.34,
    "weightG": 250
  },
  "pasta fritii 2 0": {
    "pkg": 1.32,
    "weightG": 180
  },
  "soup and toast": {
    "pkg": 20.32,
    "weightG": 200
  },
  "aglio olio": {
    "pkg": 33.34,
    "weightG": 320
  },
  "pomodoro": {
    "pkg": 33.34,
    "weightG": 370
  },
  "pesto bucatini": {
    "pkg": 33.34,
    "weightG": 270
  },
  "spicy tomato cream": {
    "pkg": 33.34,
    "weightG": 280
  },
  "alfredo": {
    "pkg": 33.34,
    "weightG": 270
  },
  "caramelised onion pasta": {
    "pkg": 20.32,
    "weightG": 300
  },
  "truffle mac cheese": {
    "pkg": 28.99,
    "weightG": 258
  },
  "pink burrata pasta": {
    "pkg": 28.99,
    "weightG": 278
  },
  "lasagna": {
    "pkg": 28.99,
    "weightG": null
  },
  "limon linguini": {
    "pkg": 33.34,
    "weightG": null
  },
  "moscow mule": {
    "pkg": 17.2,
    "weightG": null
  },
  "risotto": {
    "pkg": 33.34,
    "weightG": null
  },
  "lemon iced tea": {
    "pkg": 17.2,
    "weightG": null
  },
  "smoked tomato risotto": {
    "pkg": 28.99,
    "weightG": 230
  },
  "mint mojito": {
    "pkg": 17.2,
    "weightG": null
  },
  "pina colada": {
    "pkg": 17.2,
    "weightG": null
  },
  "mango picante": {
    "pkg": 17.2,
    "weightG": null
  },
  "melon fresca": {
    "pkg": 17.2,
    "weightG": null
  },
  "basil smash": {
    "pkg": 17.2,
    "weightG": null
  },
  "tamarind fizz": {
    "pkg": 17.2,
    "weightG": null
  },
  "sunset": {
    "pkg": 17.2,
    "weightG": null
  },
  "creamy fresca": {
    "pkg": 17.2,
    "weightG": null
  },
  "tom yum soup": {
    "pkg": 30.71,
    "weightG": 215
  },
  "chestnut gyoza 6 pcs": {
    "pkg": 35.61,
    "weightG": 115
  },
  "thai spring roll 4pcs": {
    "pkg": 27.41,
    "weightG": 185
  },
  "okonomiyaki gyoza 6pcs": {
    "pkg": 35.61,
    "weightG": 170
  },
  "kwispy lotus root": {
    "pkg": 25.77,
    "weightG": 180
  },
  "cheese chilli dumplings 5 pcs": {
    "pkg": 33.84,
    "weightG": 250
  },
  "kwispy wonton 5pcs": {
    "pkg": 27.41,
    "weightG": 75
  },
  "saucy momos 5 pcs": {
    "pkg": 33.84,
    "weightG": 175
  },
  "tteokbokki": {
    "pkg": 31.34,
    "weightG": 250
  },
  "truffle edamame dimsum 4 pcs": {
    "pkg": 30.71,
    "weightG": 150
  },
  "tofu bao 2pcs": {
    "pkg": 24.98,
    "weightG": 250
  },
  "chilli oil dumplings 5 pcs": {
    "pkg": 33.84,
    "weightG": 200
  },
  "general tso s water chestnut": {
    "pkg": 25.77,
    "weightG": 300
  },
  "steamed edamame": {
    "pkg": 25.77,
    "weightG": 160
  },
  "korean mandu": {
    "pkg": 27.41,
    "weightG": 80
  },
  "creamy corn rocks": {
    "pkg": 25.77,
    "weightG": 250
  },
  "crispy scallion pancake": {
    "pkg": 33.84,
    "weightG": 250
  },
  "cold spicy sesame noodles": {
    "pkg": 35.79,
    "weightG": 280
  },
  "tokyo style pizza": {
    "pkg": 33.84,
    "weightG": 15
  },
  "mushroom tempura": {
    "pkg": 34.95,
    "weightG": null
  },
  "vietnamese spring roll 2 pcs": {
    "pkg": 35.61,
    "weightG": 250
  },
  "summer salad": {
    "pkg": 31.34,
    "weightG": null
  },
  "avocado roll": {
    "pkg": 37.38,
    "weightG": 190
  },
  "dragon roll": {
    "pkg": 37.38,
    "weightG": 180
  },
  "volcano roll": {
    "pkg": 37.38,
    "weightG": 300
  },
  "gimbap": {
    "pkg": 37.38,
    "weightG": 350
  },
  "bombay blues sushi": {
    "pkg": 37.38,
    "weightG": 250
  },
  "fried rice": {
    "pkg": 33.84,
    "weightG": 380
  },
  "jalape o poppers": {
    "pkg": 37.38,
    "weightG": 190
  },
  "burnt garlic rice": {
    "pkg": 33.84,
    "weightG": 385
  },
  "corn tempura": {
    "pkg": 37.38,
    "weightG": 220
  },
  "mushroom truffle fried rice": {
    "pkg": 33.84,
    "weightG": 450
  },
  "avo crispy rice": {
    "pkg": 37.38,
    "weightG": 180
  },
  "kwispy edamame roll": {
    "pkg": 37.38,
    "weightG": 180
  },
  "mille feuille": {
    "pkg": 23.99,
    "weightG": 215
  },
  "chocolate coconut souffl": {
    "pkg": 18.36,
    "weightG": 155
  },
  "mango tres leches": {
    "pkg": 28.99,
    "weightG": 350
  },
  "ferrero crunch": {
    "pkg": 31.34,
    "weightG": 150
  },
  "lemon cake": {
    "pkg": 18.99,
    "weightG": 140
  },
  "katsu curry": {
    "pkg": 46.19,
    "weightG": 470
  },
  "thai curry": {
    "pkg": 46.19,
    "weightG": 500
  },
  "sri lankan red curry": {
    "pkg": 40.49,
    "weightG": 500
  },
  "tofu and mushroom curry": {
    "pkg": 40.49,
    "weightG": 500
  },
  "kwispy burmese curry": {
    "pkg": 40.49,
    "weightG": 500
  },
  "coconut curry ramen": {
    "pkg": 40.49,
    "weightG": 750
  },
  "kala khatta soda": {
    "pkg": 23.56,
    "weightG": null
  },
  "scarlett": {
    "pkg": 23.56,
    "weightG": null
  },
  "berry breeze": {
    "pkg": 23.56,
    "weightG": null
  },
  "tropical pop": {
    "pkg": 23.56,
    "weightG": null
  },
  "hakka noodles": {
    "pkg": 35.79,
    "weightG": 350
  },
  "thai lemon boba tea": {
    "pkg": 23.63,
    "weightG": null
  },
  "drunken noodles": {
    "pkg": 35.79,
    "weightG": 360
  },
  "mango schezwan chamoy": {
    "pkg": 23.56,
    "weightG": null
  },
  "buttery chilli garlic noodles": {
    "pkg": 35.79,
    "weightG": 350
  },
  "raspberry kafir fizz": {
    "pkg": 23.56,
    "weightG": null
  },
  "shoyu ramen": {
    "pkg": 50.64,
    "weightG": 600
  },
  "iyoshi cola": {
    "pkg": 12.68,
    "weightG": null
  },
  "peanut butter ramen": {
    "pkg": 35.79,
    "weightG": 350
  },
  "spiced miso ramen": {
    "pkg": 50.64,
    "weightG": 600
  },
  "pad thai": {
    "pkg": 35.79,
    "weightG": 300
  },
  "maggi udon": {
    "pkg": 33.84,
    "weightG": 400
  }
};

/** Normalize a recipe/dish name for matching (lowercase, alnum-collapsed). */
export function normDishName(s: string): string {
  return s.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
}
